...
impl AsyncExecutorTask {
        ...
        loop {
            ...
            match select(term_fut, flag_fut).await {
                Either::Left(_) => {
                    ...
                    defmt::info!("[{}: {}] The flag for (3.3) is {:?}", task_debug_name, log_task_id, flag);
                    return Ok(());
                },
                Either::Right((mut message, _)) => {
                    ...
                    if let Err(error) = message.release() {
                        defmt::info!("[{}: {}] Message could not be released: {}", task_debug_name, log_task_id, error);
                    }
                },
            }
        }
    ...
    /// This function shall use the [pxros_run] macro to spawn an async task via [crate::ex3_3::message_handler].
    ///
    ...
    pub fn ex3_3_solution(mailbox: PxMbx_t) {
        ...
        pxros_run!(mailbox, FlagEvents, PxResult<()>, Self::message_handler(flagger, termination));
    }
}
